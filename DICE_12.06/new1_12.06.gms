...
$offtext
$title October 12, 2023 (DICE2023-b-4-3-10.gms) - Modified

set        t  Time periods (5 years per period)                   /1*61/
PARAMETERS
** If optimal control
...
** Preferences, growth uncertainty, and timing
        ...
        prstp     Pure rate of social time preference               /.015/
...
** -------------------------------------------------------------------------
** 相对价格模型参数 (修改开始)
** 数据来源: Drupp & Hansel (2021) Table 1 (Sterner & Persson Replication)
** -------------------------------------------------------------------------
        alpha_n   自然资本在效用函数中的权重 (alpha)                 / 0.1 /
* 来源: S&P (2008) 假设 alpha = 0.1

        theta     替代性参数 (theta 对应文献中的 zeta)              / -1.0 /
* 来源: S&P (2008) 假设完全互补性 (Perfect Complements 倾向), theta = -1
* 注: Drupp & Hansel (2021) Table 3 的 "中心校准" (Central Calibration) 建议值为 -0.11

        psi_n     自然资本的损害参数                                / 0.0035 /
* 来源: S&P (2008) 设定非市场损害等于市场损害 (即总损害翻倍)
* DICE2023 的市场损害参数 a2base 约为 0.0035，因此此处设为相同值以实现翻倍效果

        N0        初始自然资本总量 (Total Natural Capital)          / 100.5 /
* 来源: S&P (2008) 假设 N0 = C0 (初始消费)
* 基于 DICE2023 数据: 2020年产出 q1=135.7, 储蓄率约 26%, 消费 C0 约为 100.5 万亿美元
** -------------------------------------------------------------------------
** 相对价格模型参数 (修改结束)
** -------------------------------------------------------------------------
;
...
** -------------------------------------------------------------------------
;
NONNEGATIVE VARIABLES  MIU, TATM, MAT, MU, ML, Y, YNET, YGROSS, C, K, I, RFACTLONG, N;
EQUATIONS
...
** 相对价格自然资本动态机制 (修改开始)
** -------------------------------------------------------------------------
** 使用 Sterner & Persson (2008) 的动态机制，其中自然资本 N 随温度平方 T^2 衰退
 seqN(t)..            N(t)           =E= N0 / (1 + psi_n * TATM(t)**2);
** -------------------------------------------------------------------------
** 相对价格自然资本动态机制 (修改结束)
** -------------------------------------------------------------------------
...
** 相对价格效用函数 (修改开始)
** -------------------------------------------------------------------------
** 修改为 CES 效用函数 (参照 Drupp & Hansel 2021)
** N 定义为总量，但按人均 (N/L) 形式进入效用函数，与人均消费 C/L 保持一致
** 公式: U = ( [ (1-alpha)*c^theta + alpha*(N/L)^theta ] ^ ( (1-eta)/theta ) - 1 ) / (1-eta)
** 原代码：periodueq(t)..       PERIODU(t)     =E= ((C(T)*1000/L(T))**(1-elasmu)-1)/(1-elasmu)-1; 
 periodueq(t)..       PERIODU(t)     =E= ( ( (1-alpha_n)*(C(t)*1000/L(t))**theta + alpha_n*(N(t)*1000/L(t))**theta )**((1-elasmu)/theta) - 1 ) / (1-elasmu) - 1;
